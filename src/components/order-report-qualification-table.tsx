      ...
      <tbody>
        {rows.length === 0 ? (
          <tr className="border border-[#BFBFBF]">
            <td
              colSpan={2}
              className="border border-[#BFBFBF] px-4 py-6 text-center text-slate-400"
            >
              {emptyMessage}
            </td>
          </tr>
          ...
          rows.map((row, index) => (
            <QualificationRow key={`${row.label}-${index}`} row={row} />
          ))
        )}
      </tbody>
    </table>
...
function QualificationRow({ row }: { row: SummaryPreviewRow }) {
